...
echo "$(TZ='America/New_York' date) $(basename \"$0\") - Removed default Apache page and created placeholder" >> /root/activity_log.txt
...
EOF
    ...
    if command -v a2enconf &>/dev/null; then
        ...
        echo "$(TZ='America/New_York' date) $(basename \"$0\") - Created and enabled custom Apache security configuration" >> /root/activity_log.txt
    fi
fi
...
for mod in $DISABLE_MODS; do
    ...
    echo "$(TZ='America/New_York' date) $(basename \"$0\") - Disabled unnecessary Apache module: $mod" >> /root/activity_log.txt
done
...
for mod in $ENABLE_MODS; do
    ...
    echo "$(TZ='America/New_York' date) $(basename \"$0\") - Enabled security Apache module: $mod" >> /root/activity_log.txt
done
...
if [ "$restart" == "y" ]; then
    ...
    echo "$(TZ='America/New_York' date) $(basename \"$0\") - Restarted Apache service" >> /root/activity_log.txt
