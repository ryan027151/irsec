...
echo "$(TZ='America/New_York' date) $(basename "$0") - SSH hardening script started" >> /root/activity_log.txt
echo "========================================="
...
echo "$(TZ='America/New_York' date) $(basename \"$0\") - Applied hardening settings to $SSHD_CONFIG" >> /root/activity_log.txt
...
EOF
    echo "$(TZ='America/New_York' date) $(basename \"$0\") - Added strong ciphers, MACs, and KexAlgorithms to $SSHD_CONFIG" >> /root/activity_log.txt
fi
...
if [ "$restart" == "y" ]; then
    ...
    echo "$(TZ='America/New_York' date) $(basename \"$0\") - Restarted SSH service" >> /root/activity_log.txt
    echo "[+] SSH restarted"
fi

echo "========================================="
echo "$(TZ='America/New_York' date) $(basename "$0") - SSH hardening script finished" >> /root/activity_log.txt
