...
echo "$(TZ='America/New_York' date) $(basename \"$0\") - Applied MySQL security hardening SQL commands (removed anonymous users, remote root, test DB)" >> /root/activity_log.txt
...
echo "$(TZ='America/New_York' date) $(basename \"$0\") - Hardened MySQL configuration file ($MY_CNF)" >> /root/activity_log.txt

# Set secure file permissions
chmod 644 "$MY_CNF"
echo "$(TZ='America/New_York' date) $(basename \"$0\") - Set permissions on $MY_CNF to 644" >> /root/activity_log.txt
