...
ufw --force reset
echo "$(TZ='America/New_York' date) $(basename \"$0\") - Firewall reset to clean state" >> /root/activity_log.txt

echo "[+] Setting default policies to DENY..."
ufw default deny incoming
echo "$(TZ='America/New_York' date) $(basename "$0") - Set default firewall policy to deny incoming" >> /root/activity_log.txt
ufw default deny outgoing
echo "$(TZ='America/New_York' date) $(basename "$0") - Set default firewall policy to deny outgoing" >> /root/activity_log.txt
...
ufw enable
echo "$(TZ='America/New_York' date) $(basename "$0") - Firewall enabled with whitelisted IPs" >> /root/activity_log.txt
...
ufw disable
echo "$(TZ='America/New_York' date) $(basename "$0") - Firewall disabled" >> /root/activity_log.txt
ufw --force reset
echo "$(TZ='America/New_York' date) $(basename \"$0\") - All firewall rules reset after block duration" >> /root/activity_log.txt
