...
# Log the trigger
echo "$(TZ='America/New_York' date): Wazuh triggered threat hunt - Alert: $ALERT_ID Level: $ALERT_LEVEL" >> /var/ossec/logs/active-responses.log
echo "$(TZ='America/New_York' date) $(basename "$0") - Wazuh triggered auto threat hunt (Alert: $ALERT_ID, Level: $ALERT_LEVEL)" >> /root/activity_log.txt
